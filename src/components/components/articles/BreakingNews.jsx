...
const BreakingNews = ({ items, onArticleClick }) => {
  if (!items || items.length === 0) {
    return (
      <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded">
        <p className="text-yellow-700">No breaking news available at the moment.</p>
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {items.map((item) => (
          <div 
            key={item.id} 
            className="relative rounded-lg overflow-hidden cursor-pointer transition-transform hover:-translate-y-1 shadow-md hover:shadow-lg"
